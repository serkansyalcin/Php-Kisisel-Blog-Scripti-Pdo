			...
			<div class="col-lg-8 mgr-50">
				...
				<div class="most-viewed-posts p-70">
					...
					<div class="blog-items">
						<div class="row">
							<?php	
							$query = $db->query("SELECT * FROM yazilar WHERE kategori_adi_seo = '$sonuclar' order By yazi_id ASC LIMIT 2");
							if ( $query->rowCount() ){
								foreach( $query as $row ){
									$baslik = $row["baslik"];
									$baslik_seo = $row["baslik_seo"];
						...
						<div class="post-list">
							<div class="row">
								<?php	
								$query = $db->query("SELECT * FROM yazilar WHERE kategori_adi_seo = '$sonuclar' order By yazi_id ASC LIMIT 2,14");
								if ( $query->rowCount() ){
									foreach( $query as $row ){
										$baslik = $row["baslik"];
										$baslik_seo = $row["baslik_seo"];
